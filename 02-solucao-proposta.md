...
**Escopo da garantia — dentro de um processo, não entre processos:** `ConcurrentHashMap` resolve concorrência **dentro de uma única instância da aplicação** (múltiplas threads da mesma JVM processando requisições HTTP simultâneas de balanças diferentes). Ele **não** resolveria nada se a aplicação rodasse em múltiplas instâncias/servidores ao mesmo tempo — nesse cenário, cada instância teria seu próprio mapa isolado, e duas instâncias diferentes processando a mesma balança poderiam corromper o estado, já que não compartilham memória entre si.

É exatamente aí que o Kafka entraria, caso fosse necessário escalar horizontalmente: em vez do controller chamar `StabilizationService.process()` diretamente, ele publicaria a leitura num tópico particionado por `scaleId`; o particionamento garantiria que todas as leituras de uma mesma balança sempre caem na mesma partição, consumida por uma única instância por vez — recriando, entre múltiplos processos, a mesma garantia de ordem/atomicidade por chave que o `ConcurrentHashMap.compute()` já dá hoje dentro de um processo só. A lógica do `StabilizationService` em si não mudaria — só quem o invoca (endpoint direto vs. consumer Kafka).
...
- **Migrar formato de erro para RFC 9457 (Problem Details)**: o projeto usa um `ErrorResponse` customizado (`status`, `message`, `timestamp`) via `GlobalExceptionHandler`, funcional e testado em todos os controllers. O padrão RFC 9457 (suportado nativamente pelo Spring Boot 3 via `ProblemDetail`) é o padrão de mercado reconhecido para corpos de erro HTTP e seria uma migração natural — não implementado nesta entrega por ser puramente estrutural (não altera comportamento HTTP/status codes) e exigir retrabalho de ~1h-1h30 revalidando testes e cenários manuais já cobertos pelo formato atual, tempo priorizado para a Stabilization Engine (Épico 3).
- **Estratégia de logging estruturada**: hoje o uso de log é pontual e ad-hoc (`SLF4J log.warn(...)` só nos casos de skip gracioso do `WeighingPersistenceService`; `ReadingIdempotencyService` não loga nada), sem padronização de nível por camada, correlação entre requisições, ou formato estruturado. Melhorias sugeridas para produção real:
  - Nível de log consistente por tipo de evento: `INFO` para eventos de negócio relevantes (pesagem estabilizada, transação fechada — hoje sem log nenhum de sucesso), `WARN` para gaps/skips conhecidos (já existente), `ERROR` reservado para falhas reais não recuperáveis (hoje o `catch (RuntimeException ex)` de persistência em `ScaleReadingController` não loga nada antes de relançar a exceção).
  - Correlação via MDC (Mapped Diagnostic Context) com `scaleId`/`plate` por requisição, facilitando rastrear o fluxo de uma leitura específica em meio a múltiplas balanças enviando simultaneamente.
  - Formato estruturado (JSON) em vez de texto livre, facilitando agregação/consulta em ferramentas de observabilidade (Splunk, ELK, Grafana Loki) em um cenário de produção real.
  - Não implementado nesta entrega por escopo de tempo — documentado aqui como próximo passo natural de maturidade operacional.
- **Observabilidade via Spring Boot Actuator + Prometheus**: adicionar `spring-boot-starter-actuator` + `micrometer-registry-prometheus` exporia métricas de infraestrutura (latência HTTP, uso de memória/threads, `/actuator/health`) com esforço mínimo de configuração. Para valor real além de métricas genéricas de infraestrutura, o próximo passo seria métricas de negócio customizadas via `MeterRegistry` — contador de leituras descartadas por idempotência, tempo médio até estabilização por balança, taxa de skip gracioso por tipo (transação/balança não encontrada, peso negativo). Combinaria bem com o dashboard HTML de demonstração e com a estratégia de logging estruturada acima. Não implementado nesta entrega — puramente aditivo, sem bloquear nenhum requisito do desafio; sugerido como próximo passo natural após a conclusão desta entrega.
