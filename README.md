...
- **Estoque dinâmico na doca**: `currentStock` **aumenta automaticamente conforme pesagens reais são concluídas** — cada pesagem completada representa grão que fisicamente chegou à doca, então o `netWeightKg` da pesagem é somado ao `currentStock` do tipo de grão, dentro da mesma transação (`@Transactional`) que persiste o `WeighingRecord` e fecha a `TransportTransaction`. Isso reflete a "quantidade disponível na doca" do enunciado original: como a margem é inversamente proporcional a esse estoque, ela cai à medida que entregas chegam, e o alerta de escassez é **dinâmico ao longo da vida da instância** — não é um dado fixo de demo. Pesagens que **não** completam (canceladas, ou descartadas por qualquer guarda: sem transação aberta, sem balança, tara nula, preço nulo, peso líquido não positivo) **não** alteram o estoque.

Exemplo com dados reais de `data.sql` (Milho), **no estado de boot inicial** (antes de qualquer pesagem de Milho): `purchasePricePerTon = 95.50`, `currentStock = 2000`, `maxReferenceStock = 40000` → `ratio = 0.05` → `margin = 0.20 - 0.15 × 0.05 = 0.1925` (19,25%) → `salePricePerTon = 95.50 × 1.1925 = 113.88375 ≈ 113.88`. Esses números descrevem o boot fresco e **mudam** conforme pesagens de Milho são processadas: o `currentStock` sobe, a margem cai e, quando ela fica abaixo do limiar de escassez, o Milho deixa de aparecer no alerta.

Esse valor de estoque do Milho foi deliberadamente reduzido no seed (de um valor maior para 2000) para que a margem calculada já ultrapasse o limiar de escassez (`reports.scarcity-threshold: 0.18`, em `application.yml`) desde o boot da aplicação, sem precisar de nenhuma edição manual no banco — o endpoint `/api/reports/scarcity-alerts` já mostra o Milho como alerta ativo assim que a aplicação sobe. Com o estoque agora dinâmico, esse alerta pode desaparecer ao longo de uma sessão longa, à medida que pesagens de Milho acumulam estoque e reduzem a margem abaixo do limiar.
